...
import com.twentyfourcalendar.watchface.data.model.Event
import com.twentyfourcalendar.watchface.data.model.Task
import com.twentyfourcalendar.watchface.data.repository.SunTimesRepository
...
class DataLayerCalendarRepository(
    ...
    // Lazy so Wearable client initialization (which may block connecting to Google Play
    // Services) does not run before loadFromLocalCache() in init.
    private val dataClient: DataClient by lazy { Wearable.getDataClient(context) }
    private val messageClient: MessageClient by lazy { Wearable.getMessageClient(context) }
    private val gson = Gson()

    init {
        // Load from local SharedPreferences cache synchronously for instant rendering.
        // This runs on the calling thread (main) before any async operations,
        // so _snapshot.value is already populated when the first render frame fires.
        loadFromLocalCache()
        ...
        // Also check the Data Layer for data that arrived while the watch face was closed.
    ...
    private suspend fun loadFromDataLayer() {
        try {
            ...
            val dataItems = withTimeoutOrNull(5_000L) {
    ...
    /**
     * Loads the last-known snapshot from SharedPreferences synchronously.
     * Called in init so that _snapshot.value is non-null on the very first render frame,
     * eliminating the blank watch face while waiting for phone sync (~40 s).
     */
    private fun loadFromLocalCache() {
        try {
            val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
            val json = prefs.getString(KEY_CACHED_SNAPSHOT, null) ?: return
            val dto = gson.fromJson(json, SnapshotDto::class.java)
            _snapshot.value = dto.toDailySnapshot()
            android.util.Log.d(
                "DataLayerCalendar",
                "Loaded snapshot from local cache (${_snapshot.value?.events?.size} events)",
            )
        } catch (e: Exception) {
            android.util.Log.w("DataLayerCalendar", "Failed to load from local cache", e)
        }
    }

    /**
     * Persists the raw snapshot JSON received from the phone so it is available
     * immediately on the next watch face launch, before the phone responds.
     */
    private fun saveToLocalCache(json: String) {
        try {
            context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
                .edit()
                .putString(KEY_CACHED_SNAPSHOT, json)
                .apply()
        } catch (e: Exception) {
            android.util.Log.w("DataLayerCalendar", "Failed to save to local cache", e)
        }
    }
    ...
    private fun parseFromDataItem(dataItem: com.google.android.gms.wearable.DataItem) {
        try {
            ...
            val dto = gson.fromJson(json, SnapshotDto::class.java)
            _snapshot.value = dto.toDailySnapshot()
            // Persist so the next launch can render instantly without waiting for the phone.
    ...
    companion object {
        ...
        private const val PREFS_NAME = "calendar_snapshot_cache"
        private const val KEY_CACHED_SNAPSHOT = "cached_snapshot_json"
    }
}
